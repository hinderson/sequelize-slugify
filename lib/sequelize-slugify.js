...
class SequelizeSlugify {
  slugifyModel(model, options) {
    ...
    const DEFAULT_OPTIONS = {
      ...
      paranoid: true,
      bulkUpdate: false,
      isUniqueSlug: async (slug, transaction, instance) => {
    ...
    const handleSlugify = async function (instance, sequelizeOptions, forceGenerateSlug) {
      ...
      let isUnique = await isUniqueSlug(slugValue, transaction, instance);
      if (isUnique) {
      ...
      slugValue = await addNumericSuffix(slugValue, transaction, instance);
    ...
    const isUniqueSlug = slugifyOptions.isUniqueSlug;
    ...
    const addNumericSuffix = async function (slugValue, transaction, instance) {
      let count = 1;
      let foundUnique = false;
      let suffixedSlug = '';
      while(!foundUnique) {
        suffixedSlug = `${slugValue}${slugifyOptions.incrementalSeparator}${count}`;
        foundUnique = await isUniqueSlug(suffixedSlug, transaction, instance);
        count++;
      }
      return suffixedSlug;
    };
